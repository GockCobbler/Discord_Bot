...
class Splicer:
	...
	def __init__(self, user, response, server, content, interaction):
		...
		if not os.path.isfile(self.money_path):
			df = pd.read_csv("data/templates/money_template.csv", index_col="user")
		...
		if not os.path.isfile(self.trigger_path):
			df = pd.read_csv("data/templates/trigger_pd_template.csv", index_col="username")
	...
	def from_interaction(cls, interaction):
		return cls(interaction.user, interaction.response, interaction.guild, interaction.followup, interaction)

	@classmethod
	def from_message(cls, message):
		return cls(message.author, message.channel, message.guild, message.content, None)
